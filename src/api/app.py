...
def create_app(
    ...
    if not cors_origins:
        cors_origins = list(dev_cors_defaults)
    ...
    if not is_production:
        # Local dev commonly flips between localhost and 127.0.0.1.
        # Keep both working even when the .env list is stale or partial.
        merged = list(dict.fromkeys([*cors_origins, *dev_cors_defaults]))
        if merged != list(cors_origins):
            logger.info("cors_origins_extended_for_dev", configured=cors_origins, effective=merged)
        cors_origins = merged
    else:
